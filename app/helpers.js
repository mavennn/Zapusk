...
global.postJson = function(url,POST,params = false){
	return new Promise(function(resolve, reject){
		post(url,POST,params).then(data=>{
			if(!(data instanceof Object))
				data = parseJson(data);
			if(data===false){
				resolve(erJson('Неверный ответ сервера. Обратитесь к тех поддержке1.'));
				return;
			}
			resolve(data);
		}).catch(data=>{
...
global.toProm = function (data){return new Promise(function(resolve, reject){resolve(data)})}
...
global.genToken = function(size = 30){
	 return randomString(size, 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789');
}
...
global.exec = function(command, dir = false, log_commands = false){
	return new Promise(resolve => {
		execBash( (dir ? `(cd ${dir} && ${command})` : command ), (error, stdout, stderr) => {
			if (error)
...
global.nav = function(page, lpage, elCount){
	...
	if(totalpage <= 8){
		...
		if((page<=2)||(totalpage - page <=1)){
			...
			arReady.pages.push({active:false, page: Math.ceil(totalpage/2), name: '...'});
			for(let i=(totalpage - 2); i<=totalpage; i++){
				arReady.pages.push({active:(page == i), page: i, name: i})
			}
		}else{
			arReady.pages.push({active:false, page: 1, name: 1});
			arReady.pages.push({active:false, page: Math.ceil(page/2), name: '...'});
			arReady.pages.push({active:false, page: page - 1, name: page - 1});
			arReady.pages.push({active:true, page: page, name: page});
			arReady.pages.push({active:false, page: page + 1, name: page + 1});
			arReady.pages.push({active:false, page: Math.ceil((totalpage + page)/2), name: '...'});
...
global.toPromise = function(func, context, Jtype = true) {
	return function(...args){
		return new Promise((resolve, reject) => {
			args.push((err, data) => err ? (Jtype ? resolve(erJson(err)): reject(err)) : (Jtype ? resolve(suJson(data)): resolve(data)));
