...
const AutoIncEn = class{
	...
	constructor(Schema, name, prop = 'aid'){
		...
		if(!name)
			throw new Error('AutoInc не указано name');
	...
	async getMany(count = 1){
		...
		if(count < 1)
			throw new Error('AutoIncEn getMany должно быть больше 0');
		let collect = await this.collect;
		let arIds = [];
...
global.AutoInc = new class{
	...
	async getOrCreate(name){
		...
		if(!this.collections[name])
			this.collections[name] = await autoIncModel.create({collect: name, last: 0});
