module.exports = class extends AbstractCtrl{
	...
	async exect(){
		...
		if (empty(this.speakerId))
			return erJson('Speaker ID?')
		...
		let checkMeck = await requestModel.findOne({speaker: this.speakerId, user: this.user._id, day: this.day})
		if (checkMeck)
			return erJson('You have already submitted an application')
/*
		if (!this.day)
			return erJson('Day?')
*/		
		...
		if (speaker.recording_status[`day${this.day}`] == 2) {
			return (await requestManager.create({userId: this.user._id, speakerId: this.speakerId, day: this.day}))
		}
...
*/
		...
		if (speaker.recording_status[`day${this.day}`] == 3) {
			...
			let meetings = await meetingModel.find({speaker: this.speakerId, day: this.day})
			let all_meetings = await meetingModel.find({day: this.day})
			let time_checked = {}
			all_meetings.map(val => {
				if(!time_checked[val.time])
					time_checked[val.time] = []
			...
			for(let kek = 0; kek < 10; kek++) {
				...
				if(kek < 9)
					if (time_checked[times_today[times_today.indexOf(meetings[kek].time) + 1]].indexOf(request.user) >= 0 ) {
						valid = false
					}
			...
			if (valid) {
				...
				return erJson("Sorry, you signed up for the maximum number of appointments")
			}
		}
