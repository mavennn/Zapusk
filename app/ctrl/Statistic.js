...
*/
		...
		for (let val of requests) {
			if(!empty(val.user) && !empty(val.speaker)){
				let meet = await meetingModel.findOne({user: val.user, speaker: val.speaker})
				if (empty(meet)) {
					let usni = users[val.user]
					usni.speaker = speakersById[val.speaker].name + ' ' + speakersById[val.speaker].sname
		...
		return suJson({ speakersById, speakers, count_users, count_meetings, users, tags, users_not_in })
