...
global.Mail = new class{
	...
	async send({title = 'Registration data for WSK2019', to = false, html = '', attachment = false } = {}){
		if(empty(to))
			return erJson('Не выбраны отправители');
	...
	sendMail(mailOptions){
		return new Promise(resolve=>{
			this.transport.sendMail(mailOptions, (error, info) => {
				console.log(error)
			    if(error)
			        return resolve(erJson('Сообщение не отправлено'));
