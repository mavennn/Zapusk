global.requestManager = new class{
	...
	async create({userId, speakerId, day}){
		...
		if (empty(speakerId))
			return erJson('Speaker?')

		let checked = await requestModel.findOne({speaker: speakerId, user: userId, day})
		if (!empty(checked))
			return erJson('You have already submitted a request')

		let user = await userModel.findOne({_id: userId, permission: ['user','admin']})
		
		if (empty(user))
			return erJson('User not found')

		let speaker = await userModel.findOne({_id: speakerId, permission: 'speaker'})
		
		if (empty(speaker))
			return erJson('Speaker not found')

		if (empty(speaker.partnership)) 
		...
		if (!empty(speaker.partnership) && speaker.partnership.role.length && !empty(user.organization)) {
		...
		if (!empty(speaker.partnership) && speaker.partnership.partnershipMode.length && user.partnership.partnershipMode.length) {
		...
		if (!empty(speaker.partnership) && speaker.partnership.industry.length && user.partnership.industry.length) {
		...
		if (!empty(speaker.bestPractice) && speaker.partnership.bestPractice.length && user.partnership.bestPractice.length) {
		...
		if (speaker.recording_status[`day${day}`] == 4){
			params.status = 3
		}
