global.meetingManager = new class{
	...
	async calc({day, time_start, speaker}){
		if(empty(speaker) || empty(time_start))
			return erJson("Time? Speaker?")
		...
		for(let i=0; i < 9; i++){
			if(i % 4 || i == 8 || i==0)
				time_man.push(moment(time_man[i],"HH:mm").add(10, 'm').format("HH:mm"));
			else
				time_man.push(moment(time_man[i],"HH:mm").add(15, 'm').format("HH:mm"));
		}
		let com = []
	...
	async setScheduleRecord({speakerId, day}){
		...
		let requests = await requestModel.find({speaker: speakerId, day: day}).sort({rating: -1})
		...
		for(let key in requests) {
			if(key != 'byKey' && key != 'byKeyArr' && key != 'unique') {
				...
				for(let kek = 0; kek < 10; kek++) {
					...
					if (time_checked[meetings[kek].time].indexOf(requests[key].user) >= 0) {
						valid = false
					}
					
					if(kek < 9)
						if (time_checked[times_today[times_today.indexOf(meetings[kek].time) + 1]].indexOf(requests[key].user) >= 0 ) {
							valid = false
						}
