...
global.tagsManager = new (class {
  ...
  async create({ name, type, weight, hidden = false }) {
    if (empty(name)) return erJson("name?");

    if (empty(type)) return erJson("type?");

    if (empty(weight)) return erJson("weight?");
  ...
  async checkTags(name) {
    if (!empty(this.tags.filter(val => val.name == name))) return true;
    else return false;
  }
