global.scheduleManager = new class{
	...
	async removeItem(id) {
		...
		if (!sch)
			return erJson('Not found')
		await sch.remove()
		return suJson('su')
	}
	
	async create({day, time, title, speaker, type}){
		if (empty(day) || day > 2 || day < 0)
			return erJson('Day?')
		...
		if (type == 'speaker' && !empty(speaker._id)){
			speaker = await userModel.findOne({ _id: speaker._id, permission: 'speaker' })
			if (empty(speaker))
		...
		let params = {
			...
			speaker: {
				...
				name: speaker.name,
				sname: speaker.sname,
				organization: speaker.organization.name
			},
			type,
		}
		...
		if(empty(schedule))
			return erJson('Error with create');
