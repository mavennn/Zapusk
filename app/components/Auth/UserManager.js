...
global.UserManager = new class{
		...
		for(let item of file){
			if(item[1]){
				...
				let user = {
					...
					sname: item[2],
					prefname: item[2],
					country: ((tagsManager.country.filter(v => v.name == item[3]))[0] || {})._id,
					city: item[4],
					phone: item[9],
					prefix: item[0],
					photo: item[12] ? 'https://getfile.dokpub.com/yandex/get/' + item[12] : '',
					presentation: {
						subject: item[10],
						link: item[11],
					},
				...
				if(!ch){
					...
					ch.country = (tagsManager.country.filter(v => v.name == item[4]))[0]._id
					ch.photo = item[12] ? 'https://getfile.dokpub.com/yandex/get/' + item[12] : ''
					await ch.save()
					this.addToCache(ch)
				}
			}
		...
		for(let item of file){
			if(item[2]){
				...
				let user = {
					...
					prefname: item[2],
					sname: item[3],
					country: ((tagsManager.country.filter(v => v.name == item[4]))[0] || {})._id,
					city: item[5],
					organization: {
						name: item[6],
						position: item[7],
						role: ((tagsManager.country.filter(v => v.name == item[8]))[0] || {})._id,
					},
					email: item[9],
					phone: item[10],
					password,
	...
	async auth(ids, password){
		...
		if(empty(ids))
			return erJson('Enter email');
		
		if(empty(password))
			return erJson('Enter password');
		
		let User = await userModel.findOne({ids});

		if(empty(User))
			return erJson('User not found');
		
		if(!User.auth(password))
			return erJson('Wrong password');
	...
	async adminAuth(email,muser){
		...
		if(muser.permission != "admin")
			return erJson("Ну перестань");
		
		let User = await userModel.findOne({email});

		if(empty(User))
			return erJson('Неверный email');
	...
	async edit(id, arUser){ //5d5c2a0a4832b09f306441b4
		...
		if (!empty(arUser.wsrProducts))
			user.wsrProducts = arUser.wsrProducts
	...
	async add(arUser){ //5d5c2a0a4832b09f306441b4
	...
	async passwordRecovery(email){
		...
		let User = await userModel.findOne({email});
		if(empty(User))
			return erJson('Пользователь не найден');
		...
		let arPost = {
			to: email,
			html: `<div>Ссылка для восстановления пароля: https://${config.get("host.url")}/recovery/${token}</div>`
		};
	...
	async changePassword(token,newpass){
		...
		if(empty(newpass))
			return erJson('Не указан пароль');
		
		
		let user = await userModel.findOne({_id: checktok.user_id});

		if(empty(user))
			return erJson("Тех ошибка");
	...
	async setHiddenTags({userId, tagsId}) {
		...
		if (empty(tags)) 
			return erJson('Tags not found')

		if (empty(user.hidden_tags))
			user.hidden_tags = []
