...
User.methods.encryptPassword = function(password) {
	return crypto.createHmac('sha1', this.salt).update(String(password)).digest('hex');
};
...
User.statics.regUser = async function({name, sname, email, password, phone}){
	try{
		...
		if(empty(name))
			return erJson('Enter your name');
		
		if(empty(password))
			password = genPass('number', 4)
		
		if(empty(email))
			return erJson('Please enter your email');
