<template>
  <div class="mpage">
    <div v-for="(page, key) in pages">
      <div
        class="mpage_block"
        :class="{ open_animation: page.in_open, close_animation: page.close }"
      >
        <div
          v-if="page.type == 'standart' && use_header"
          class="mpage_block_scroll"
        >
  ...
  methods: {
    open({
      ...
      if (this.closePromice)
        return this.closePromice.then(() =>
          this.open({ type, title, data, component, use_header })
        );
      if (!component) throw new Error("Неверный компонент");
      this.counter++;
      this.opened = true;
      if (this.counter == 1) {
      } else {
    ...
    closeAll({ fast = false } = {}) {
      ...
      if (this.pages.length == 1) return this.closeLast();
    ...
    closeLast() {
      ...
      let closePromice = timer(0.5).then(data =>
