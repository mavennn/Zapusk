					...
					<div class="sbb-item" v-for="(event,key) in block.events" v-bind:key="'event_'+key">
						...
						<div class="sbbi-desc">
							<div class="sbbid-row">
								...
								<div v-if="block.title == 'meetings' || block.title == 'Встречи'" class="sbbidr-time" :class="{'sbbidrt-active': event.future}">{{event.time}}</div>
	...
	export default {
		...
		created(){
            ...
            App.User.getTags()
                ...
                .then(() => {
                    App.User.getUserById(this.data)
                    .then(res=>{
                        let data = { user: res}
		...
		methods: {
                getMyMeetings() {
                    Request.postJson('/api/admin/meetings/list', { speakerId: this.data, day: this.active_day, status: 1 }).then(data => {
                        ...
                        this.blocks[0].events = data.meetings.map(val => {
	                        if (this.users[val.user]) {
	                            ...
	                            val.organization = this.users[val.user].organization ? this.users[val.user].organization.name : ''
                ...
                getMyRequests() {
                    Request.postJson('/api/admin/requests/list', { speakerId: this.data, day: this.active_day }).then(data => {
                        this.users = data.users
