	...
	<div>
		...
		<template v-if="!load" >
			<div class="sellist_vser_block" >
				...
				<input class="form-control input-sm" placeholder="Поиск по значениям" v-if="(multi || !val) && !add_mode" v-model="search" >
				
				<div v-if="add_mode" class="sellist_add_mode" >
					<input class="form-control input-sm" placeholder="Новое значение" v-model="search" >
					<button class="btn btn-success btn-sm" @click="addVal" >Добавить</button>
				</div>
				
				<div v-if="add" class="sellist_addbtn" :class="{'text-success': !add_mode, 'text-default': add_mode}" @click="add_mode = !add_mode"  >
				...
				<div class="sellist_ft_block"  v-if="multi && vals.length"  >
					<template>
						<div class="sellist_ft_sel_item" v-for="item in vals" >
							...
							<div class="sel_fib_remove" @click="removeSel(item.key)" ><i class="fa fa-times"></i> </div>
						</div>
					</template>
				</div>
				
				<button v-if="!multi && val" class="btn btn-success btn-sm waves-effect waves-light" >
					<span class="btn-label" @click="removeSel" ><i class="fa fa-remove"></i></span>
			...
			<div class="selli_block_list" >
				<div v-for="item in listItems"  >
					<div class="selli_valsli" :class="{selected: selected[item.key]}" @click="select(item)" >{{item.val}}</div>
				</div>
				<h4 v-if="!listItems.length" class="text-center" >Значений не найдено</h4>
		...
		created(){
			...
			this.SelectList.list()
				.then( data => {
					...
					if(this.data.val){
						if(this.multi){
							this.vals = this.data.val.map(val => this.list.find(item => item.key == val))
							this.vals.forEach(val => (this.$set(this.selected, val.key, true)));
						}else
							this.val = this.list.find(item => item.key == this.data.val);	
					}
			...
			select(item){
				if(!this.multi)
					...
					if(this.selected[item.key])
						return this.removeSel(item.key);
		...
		computed:{
			listItems(){
				if(this.search.length < 3)
					return this.list;
				return this.list.filter(val => val.val.search(new RegExp(this.search, 'i')) > -1 );
			}
			//reg.test(str)
		},
