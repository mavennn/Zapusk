		...
		<div class="sb_user">
			...
			<template v-else>
				...
				<div v-if="remove" class="sb_remove" >
					<button @click="onRemove" class="btn btn-danger btn-circle"><i class="fa fa-close"></i> </button>
				</div>
			</template>
		</div>
		...
		methods:{
			open() {
				...
				if (empty(this.id))
					...
					this.openClient(this.id);
				else if (this.type == 'Contact')
					this.openContact(this.id);
				else if (this.type == 'Lead')
					this.openLead(this.id);
				else if (this.type == 'Object')
					this.openObject(this.id);
				else if (this.type == 'Building')
					this.openBuilding(this.id);
				else if (this.type == 'Complex')
					this.openComplex(this.id);
				else if (['User','Assigned','Observer', 'Subass','SubAssigned'].includes(this.type))
