                    ...
                    data-page-size="7">
                        ...
                        <tbody>
                            <tr v-for="speaker in speakers" :key="speaker._id">
                                <td>
                                    <button type="button" class="btn btn-warning btn-circle btn-lg" @click="openEdit(speaker._id)"><i class="fa fa-edit"></i></button>
                                    <button type="button" class="btn btn-success btn-circle btn-lg" @click="openDetail(speaker._id)"><i class="fa fa-eye"></i></button>
                                </td>
                                <td>{{speaker.ids}}</td>
                                <td>{{speaker._id}}</td>
                                <td>{{speaker.name}}</td>
                                <td>{{speaker.sname}}</td>
                                <td><span v-if="speaker.organization">{{speaker.organization.name}}</span></td>
                                <td>
...
<script>
	...
	export default {
		...
		methods: {
            getSpeakers() {
                Request.postJson('/api/admin/users/list', { type: 'speaker'}).then(data => {
                    this.speakers = data
                })
            },
            openEdit(id) {
