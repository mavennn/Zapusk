...
global.findAddAddress = function(query, withoutHome = false) {
	return new Promise( (resolve, reject) => {
		Request.postNoCredits(`https://dadata.ru/api/v2/suggest/address`,
		{
			query,
			count: 1,
		...
		.then(data => {
			...
			let suggestion = Object.assign({}, (data.suggestions[0] ? data.suggestions[0].data : {}) );
			...
			let ModalAddress = {
				...
				block: `${suggestion.block_type || ''}${suggestion.block || ''}` || false,
				canContinue: true,
				coords: {
...
global.printPDF = function(type,orderId,params = {}) {
	...
	Load.start();
	return Request.postJson(Request.url.orders('/api/operator/order/' + orderId + '/print-pdf'), printParams)
		.then(data => { 
...
global.swalAskText = (title = 'Подтверждение', text = '', {yes = 'Да', no = 'Нет', can_empty = false} = {}) => {
