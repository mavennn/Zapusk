...
export default new (class {
  ...
  Component(name, recurse = 0) {
    ...
    if (!name) throw new Error("Load Component no name");
    ...
    this.loadedComponents[name] = new Promise(res => {
      ...
      this.loadedComponents[name] = import(`@Components/${name}.vue`)
        .then(Comp => {
        ...
        .catch(data => {
          ...
          else this.loadedComponents[name] = this.Component(name, recurse);
        });
    });
  ...
  Module(name) {
    ...
    if (this.loadedModules[name]) return this.loadedModules[name];
