global.Xedit = class xedit {
  ...
  push(arProp) {
    if (Array.isArray(arProp)) arProp.forEach(val => this.push(val));
    else this.props[arProp.prop] = arProp;
  }
  ...
  edit(prop, current, event, exdata = {}) {
    if (
      ...
      this["type" + this.props[prop].type] == undefined
    ) {
    ...
    let val;
    return this["type" + prop.type](prop, current, coords)
      .then(data => {
        val = data;
  ...
  save(prop, val, exdata) {
    this.close();
    if (prop.save != undefined) return prop.save(prop, val, exdata);
    let code = prop.code || prop.prop;
    let arPost = { code, val: val.val, id: this.id };
    if (prop.save_params) arPost = Object.assign(arPost, prop.save_params);
  ...
  openPop(title, content, coords) {
    ...
    this.pop = $('<div class="xedit_place" ><div id="xedit_back" ></div></div>')
      .appendTo("body")
      .css(coords)
      .webuiPopover({ title, content, dismissible: false });
  ...
  eventCheck(prop, current, fval) {
    return new Promise((res, rej) => {
      $("#xedit_check").click(() => {
        ...
        if (prop.is_price) val.val = val.val.replace(/\s+/g, "");

        if ((prop.is_number || prop.is_price) && !isNumber(val.val))
          return $("#xedit_validate_place").html(
            `<b>Ошибка:</b> Значение должно быть числом с разделителем .`
          );
  ...
  print(prop, val) {
    ...
    let valRet = this["type" + this.props[prop].type + "print"](
      this.props[prop],
      val
    );
  ...
  typetext(prop, current, coords) {
    ...
    if (prop.after_show) prop.after_show("#xedit_inp_val");
  ...
  typetextprint(prop, val) {
    if (prop.print) val = prop.print(val);
  ...
  typeselect(prop, current, coords) {
    ...
    content += "</select></div>";
    content +=
      '<div class="xedit_select_btns" >' + xedit.getButtons() + "</div>";

    let arTags = this.getSelectVals(prop).map(val => {
      return { id: val.key, text: val.val };
    });
    if (prop.select_add)
    ...
    if (prop.select_add)
      select2.on("select2:select", () => {
        if (select2.val() == "add_new_select")
          this.typeSelectAdd(prop, select2);
      });

    if (prop.after_show) prop.after_show("#xedit_select_val");
  ...
  typeSelectAdd(prop, select2) {
    return swal({
      ...
      preConfirm: val => {
        if (empty(val)) return Promise.reject("Поле не модет быть пустым");
  ...
  typeselectprint(prop, val) {
    return (
      this.getSelectVals(prop).find(el => el.key == val) || {
        val: "Не выбрано"
      }
    ).val;
  }
  ...
  typeselectGetVal(prop) {
    ...
    val = this.getSelectVals(prop).find(el => el.key == val) || {
      key: 0,
      val: "Не выбрано"
    };
  ...
  typemultiselect(prop, current, coords) {
    ...
    if (prop.select_add)
      select2.on("select2:select", () => {
        if (select2.val().indexOf("add_new_select") > -1)
          this.typeMultiselectAdd(prop, select2);
      });
  ...
  typemultiselectprint(prop, val) {
    if (!Array.isArray(val)) return "Не выбрано";
