        ...
        <div class="sm-body">
            <table class="smb-table">
                <thead>
                    <tr>
                        <th>SPEAKERS</th>
                        <th :class="{'smbt-current': currentTime(t,times[key+1]?times[key+1]:t)}" v-for="(t,key) in times" v-bind:key="'t'+key">{{t}}</th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-for="(speaker,key) in speakers" v-bind:key="'sp'+key">
                        <td>{{getName(speaker.id)}}</td>
                        <td  :class="{'smbt-current': currentTime(user.time, speaker.users[key+1]?speaker.users[key+1].time: user.time)}" v-for="(user,key) in speaker.users" v-bind:key="'sp'+key">{{user.id?getName(user.id):'-'}}</td>
                    </tr>
                </tbody>
            </table>
        </div>
	...
	export default {
		...
		methods: {
            currentTime(curr,nex){
                ...
                return Number(curr.replace(':','')) <= numval && numval < Number(nex.replace(':',''))
            },
            getName(id){
                if(this.names[id]){
