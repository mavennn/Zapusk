	...
	<div class="skittles-profile">
		<div class="sp-block">
            ...
            <div class="spb-content" v-if="opts.country">
                ...
                <div class="spbc-inputgroup">
                    <div class="spbcig-label">{{lang == 'en'?'Family name':'Фамилия'}}*:</div>
                ...
                <div class="spbc-inputgroup">
                    <div class="spbcig-label">{{lang == 'en'?'City':'Город'}}*:</div>
                    <input v-model="user.city" class="spbcig-val" type="text" :placeholder="lang == 'en'?'Input your city':'Введите город'" />
                </div>
                <div class="spbc-inputgroup">
        ...
        <div class="sp-save" @click="save()">{{lang == 'en'?'save':'сохранить'}}</div>
        <CompDesc :lang="lang" />
	</div>
	...
	export default {
		...
		methods: {
			...
			upload(file){
				...
				xhr.open("POST", "/api/user/loadphoto/"+App.User.arUser._id+'.'+filena , true);
				xhr.send(file);
			},
            togglePrefix(){
