<template>
	...
	<div class="skittles-sign" :class="{'ss-light':tab == 'reg'}">
		...
		<div class="ss-toggler">
			<div class="sst-item" :class="{'ssti-active': tab == 'login'}" @click="tab = 'login'">{{lang=='en'?'sign in':'войти'}}</div>
			<div class="sst-item" :class="{'ssti-active': tab == 'reg'}" @click="tab = 'reg'">{{lang=='en'?'registration':'регистрация'}}</div>
		</div>
...
<style lang="scss">
	...
	.skittles-sign{
		...
		&.ss-light{
			...
			.ss-logo{
				background: url(/src/build/img/wsrlogo-01.svg) center no-repeat;
				background-size: contain;
				width: 189px;
				height: 147px;
