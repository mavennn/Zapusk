<template>
	...
	<div class="skittles-reg">
		<div class="sr-block" v-if="step == 0">
			...
			<div class="sr-input">
				<div class="sri-label">{{lang =='en'?'Family name':'Фамилия'}}*:</div>
				<input class="sri-val" v-model="user.sname" />
			</div>
			<div class="sr-input">
				<div class="sri-label">E-mail ({{lang =='en'?'not visible to public':'не виден остальным'}})*:</div>
				<input class="sri-val" v-model="user.email" />
			</div>
			<div class="sr-input">
				<div class="sri-label">
					{{lang =='en'?'Phone number (not visible to public)':'Телефон (не виден остальным)'}}:</div>
			...
			<div class="sr-policy">
				...
				<div class="sr-links">
					...
					<a href="https://wsk2019.ru/ru/policy" target="_blank">политики конфиденциальности</a> 
					</template>
					<template v-else>
						I agree to 
						<a href="https://wsk2019.ru/policy" target="_blank">privacy policy</a>
					</template>
				</div>
			</div>
		</div>
		<div class="sr-block" v-if="step == 1">
			<div class="sr-title">{{lang =='en'?'Set the pin code':'Задайте pin код'}}</div>
			<div class="sr-pin"><span v-for="(i,key) in user.password.split('')" v-bind:key="key">* </span></div>
			<div class="sr-buttons">
				<div class="srb-item" v-for="(n,key) in buttons" v-bind:key="'n_'+key" @click="enterNum(n)">{{n}}</div>
		...
		<div class="sr-block" v-if="step == 2">
			...
			<div class="sr-label">{{lang =='en'?'your pin code':'ваш pin код'}}:</div>
			<div class="sr-val">{{user.password}}</div>
			<div class="sr-advice">
				{{lang =='en'?'take a screenshot to avoid losing login info':
				'сделайте скриншот во избежание утери id и pin кода'}}</div>
		</div>
		<div class="sr-next" v-if="step != 1" @click="nextStep()">
			<template v-if="step < 2">{{lang =='en'?'next':'далее'}} <i class="fa fa-angle-right" aria-hidden="true"></i></template>
...
<script>
	export default {
		...
		methods: {
			...
			delNum(){
				this.user.password = this.user.password.substring(0, this.user.password.length - 1)
			},
			nextStep(){
				switch(this.step){
					case 0:
					...
					case 2:
						App.User.login(this.id,this.user.password)
							.then(()=>{
								location.href = '/edit'
							})
						break;
