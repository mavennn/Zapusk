	...
	<div>
		...
		<div class="skittles-content">
			<div class="skittles-days">
				<div class="sd-item" :class="{'sdi-active': day == active_day}" v-for="(day,key) in days" v-bind:key="key" @click="active_day = day">{{lang=='en'?'Day':'День'}} {{day}}</div>
			</div>
			<div class="skittles-block" v-for="(block,key) in blocks" v-bind:key="'block_'+key">
				<div class="sb-head">
					<div class="sbh-title">{{block.title}}</div>
					<div class="sbh-place">{{block.place}}</div>
				</div>
				<div class="sb-body">
					<div class="sbb-item" v-if="event.day == active_day" v-for="(event,key) in block.events" @click="openSpeakers(block.title, event)" v-bind:key="'event_'+key">
						...
						<div class="sbbi-person">
							<div class="sbbip-name">{{event.speaker.name}} {{event.speaker.sname}}</div>
							<div class="sbbip-company">&nbsp;| {{event.speaker.organization}}</div>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="schedulled-content">
			<div class="skittles-block" v-for="(block,key) in blocks1" v-bind:key="'block_'+key">
				<div class="sb-head">
					<div class="sbh-title">{{block.title}}</div>
					<div class="sbh-place">{{block.place}}</div>
				</div>
				<div class="sb-body">
					<div class="sbb-item" v-for="(event,key) in block.events" v-bind:key="'event_'+key">
						...
						<div class="sbbi-desc">
							<div class="sbbid-row">
								...
								<div v-if="block.title =='my meetings'" class="sbbidr-time" :class="{'sbbidrt-active': event.future}">{{event.time}}</div>
								<div v-else class="sbbidr-time">
									{{event.status == 1?(lang=='en'?'Submitted':'Подана'):''}}
									{{event.status == 2?(lang=='en'?'Accepted':'Принята'):''}}
									{{event.status == 3?(lang=='en'?'Rejected.':'Отклонена'):''}}
								</div>
							</div>
							<div class="sbbid-company">{{event.organization}}</div>
						</div>
					</div>
					<div v-show="lang=='en'" class="sbb-noevent" v-if="block.events.length == 0">
						You have no any {{block.title =='my meetings'?'meetings':'requests'}} yet <br>Tap <span>SPEAKERS</span> for making appointments
					</div>
					<div v-show="lang=='ru'" class="sbb-noevent" v-if="block.events.length == 0">
						Нажмите на вкладку <span>СПИКЕРЫ</span>, чтобы записаться на встречу'
					</div>
				</div>
			</div>
		</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getMyMeetings() {
				Request.postJson('/api/user/meetings/forUser', {day: this.active_day}).then(data => {
					this.users = data.users
					this.blocks1[0].events = data.meetings.map(val => {
						if (App.User.isSpeaker()){
							val.username = this.users[val.user].name +' '+ this.users[val.user].sname
			...
			getMyRequests() {
				Request.postJson('/api/user/requests/forUser', {day: this.active_day}).then(data => {
					this.users = data.users
