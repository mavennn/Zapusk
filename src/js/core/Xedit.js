global.Xedit = class xedit{
	...
	*/
	...
	eventCheck(prop, current, fval){
		return new Promise((res, rej)=>{
			$('#xedit_check').click(()=>{
				...
				if(prop.is_number && !isNumber(val.val))
	...
	typeselect(prop, current, coords){
		...
		let arTags = prop.values.map(val=>{return {id: val.key, text: val.val}});
		if(prop.select_add)
	...
	typeselectprint(prop, val){
		return (prop.values.find(el=> el.key==val) || {val:'Не выбрано'}).val;
	}
	
	typeselectGetVal(prop){
		let val = $('#xedit_select_val').val();
		val = (prop.values.find(el=> el.key==val) || {key:0, val:'Не выбрано'})
	...
	typemultiselectprint(prop, val){
		...
		val = val.map(id => {
			return (prop.values.find(enet => enet.key == id) || {}).val;
		})
	...
	typesmultiselectGetVal(prop){
		...
		let arPrint = [];
		val.forEach(id => {
			let item = prop.values.find(enet => enet.key == id);
			arVal.push(id);
