export default new class{
	...
	login(login, password){
		if(empty(login))
			return swal('Error', 'Enter Email', 'error');
		
		if (empty(password))
			return swal('Error', 'Enter password', 'error');
			
		Load.start();
		return Request.postJson('/api/auth', {login, password}, false, true).then(data=>{
			Load.stop();
			this.regInApp(data);
			return suJson();	
		}).catch(data=>{
	...
	forgot(email){
		...
		Load.start();
		Request.postJson('/api/forgot', {email}, false, true).then(data=>{
	...
	changePass(token,pass){
		...
		Load.start();
		return Request.postJson('/api/changepass', {token,pass}, false, true).then(data=>{
			return location.href = "/";	
		}).catch(data=>{
	...
	getUserList(page = false, lPage = false, search = false, active_role = false){
		...
		Load.start();
		return Request.postJson('/api/user/user-list', arPost).then(data=>{
			Load.stop();
			return data;
    ...
    createUser(User) {
		if (empty(User.name) || empty(User.sname) || empty(User.email) || empty(User.password)) 
		...
		return Request.postJson('/api/reg', arUser, false, true).then(data=>{
			Load.stop();
			this.regInApp(data);
			return data
		}).catch(requestError);
		...
		getUserInfo(){
			return Request.getJson('/api/user/get').then(data=>{
				Load.stop();
				return data
			}).catch(requestError);
		}
		...
		getSchedulle(){
			return Request.post('/api/user/schedule/list',{}, false, true).then(data=>{
				Load.stop();
				return ans(data)
			}).catch(requestError);
		}
