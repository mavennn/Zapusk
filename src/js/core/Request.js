...
global.Request = new class{
	...
	constructor(){
		...
		this.url = {
			...
			admin: (url) => Config.url('admin', url),  
		...
		return new Promise((resolve, reject) => {
			this.post(url, arData, params, no_token).then(data=>{
				if(!data instanceof Object){
					data = parseJson(data);
					if(!data)
						return reject('Неверный ответ сервера. Обратитесь к тех поддержке.');
				}
				if(isJson(data))
		...
		return new Promise(res => {
			Promise.resolve().then(data=>{
				...
				if(status == 404 || status == 502){
					swal('Ошибка', 'Наш сервер времено недоступен( повторите запрос позже', 'error');
	...
	get(url, params = false, no_token = false, count = 0) {
		let Prom;
		if(this.oldMode)
			Prom = this.getXhr(url, params, no_token);
		else
			Prom = this.getFetch(url, params, no_token);
	...
	createLC(){
		this.LCPromice = new Promise(res => {
			if(this.LCVue)
				this.LCVue.$data.show = true;
			else
				this.LCVue = new Vue(LostConnection).$mount('#lost-connection');
			this.LCResolve = res;
		}).then(data=>{
