...
global.config = new (class {
  constructor() {
    ...
    if (fs.existsSync(path)) this.config = require(path);
  ...
  url(module, url, region = false) {
    ...
    return (
      "http://" + (region ? `r${region}-` : "") + this.modulesUrl[module] + url
    );
  }
  ...
  loadEnv() {
    ...
    if (!empty(env.NJ_HOST_PORT)) {
      if (empty(this.config.host)) this.config.host = {};
      this.config.host.port = env.HOST_PORT;
    }

    if (!empty(process.env.NJ_MODE)) this.config.mode = env.NJ_MODE;

    if (empty(this.config.db)) this.config.db = {};

    if (!empty(env.NJ_DB_HOST)) this.config.db.host = env.NJ_DB_HOST;

    if (!empty(env.NJ_DB_PORT)) this.config.db.port = env.NJ_DB_PORT;

    if (!empty(env.NJ_DB_NAME)) this.config.db.db_name = env.NJ_DB_NAME;
  }
